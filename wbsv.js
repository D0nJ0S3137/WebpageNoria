...
app.post('/updateFromSniffer', (req, res) => {
  ...
  console.log(`Received data - Fecha: ${Date}, Hora: ${Time}, Latitud: ${Latitude}, Longitud: ${Longitude}`);

  // Determinar la tabla y los campos adecuados según si RPM está presente
  const table = 'Noria_database';
  const fields = '(Latitude, Longitude, Date, Time)';
  const placeholders = '(?, ?, ?, ?)';
...
app.get('/database-datos', (req, res) => {
  ...
  dbConnection.query('SELECT Latitude, Longitude, Date, Time FROM Noria_database ORDER BY ID DESC', (err, results) => {
    if (err) {
    ...
    function sendLatestDataToClients() {
      dbConnection.query('SELECT * FROM Noria_database ORDER BY ID DESC LIMIT 1', (err, results) => {
        if (err) throw err;
  if (results.length > 0) {
